...
// ==================== 8. دوال مساندة مبدئية (مكانك تعدلها أو تستبدلها بنسختك الكاملة) ====================
function refreshDropdowns() {
  SpreadsheetApp.getUi().alert('🔄 تحديث القوائم يعتمد حالياً على إعادة إنشاء الجزء 1 (setupPart1). يمكن لاحقاً عمل منطق أدق.');
}

function showUpcomingPayments() {
  SpreadsheetApp.getUi().alert('🔔 شاشة عرض الاستحقاقات التفصيلية يمكن إضافتها لاحقاً. حالياً البيانات موجودة في "التنبيهات والاستحقاقات".');
}

function updateAlerts() {
  SpreadsheetApp.getUi().alert('⚠️ منطق تحديث التنبيهات لم يُنقل هنا بعد. يمكنك الإبقاء على نسختك القديمة أو تطوير منطق جديد.');
}

function showGuide() {
  SpreadsheetApp.getUi().alert(
    '📖 دليل مختصر:\n\n' +
    '1) استخدم "إنشاء النظام - الجزء 1" لأول مرة فقط (يحذف كل الشيتات ويعيد بناءها).\n' +
    '2) سجّل الاستحقاقات والمدفوعات من القائمة.\n' +
    '3) استخدم دفتر الحركات لمراجعة الرصيد لكل طرف.\n' +
    '4) الميزانيات تعتمد على القيمة بالدولار للحركات المدينة.'
  );
}

// بقية الدوال (التقارير التفصيلية، كشف حساب المورد/العميل/الممول، ربحية المشاريع، الفواتير...)
// يمكنك ترك النسخة القديمة منها أسفل هذا الكود أو نقلها كما هي بدون تعديل.
...
function createExpenseReportSheet(ss) {
  ...
  sheet.getRange('A2').setFormula(
    `=UNIQUE(FILTER('دفتر الحركات المالية'!G2:G,'دفتر الحركات المالية'!G2:G<>""))`
  );
  
  // باقي الأعمدة تعتمد على A (تشتغل على J/K زي ما هي في الشيت)
  for (let row = 2; row <= 100; row++) {
    // التصنيف
    sheet.getRange(row, 2).setFormula(
      `=IF(A${row}="","",IFERROR(INDEX('دفتر الحركات المالية'!D:D,MATCH(A${row},'دفتر الحركات المالية'!G:G,0)),""))`
    );
    // إجمالي المستحق (استحقاق مصروف) من J:J
    sheet.getRange(row, 3).setFormula(
      `=IF(A${row}="","",SUMIFS('دفتر الحركات المالية'!J:J,'دفتر الحركات المالية'!G:G,A${row},'دفتر الحركات المالية'!C:C,"💰 استحقاق مصروف"))`
    );
    // المدفوع (دفعة مصروف) من K:K
    sheet.getRange(row, 4).setFormula(
      `=IF(A${row}="","",SUMIFS('دفتر الحركات المالية'!K:K,'دفتر الحركات المالية'!G:G,A${row},'دفتر الحركات المالية'!C:C,"💸 دفعة مصروف"))`
    );
    // المتبقي
    sheet.getRange(row, 5).setFormula(
    ...
    // عدد الاستحقاقات
    sheet.getRange(row, 6).setFormula(
      `=IF(A${row}="","",COUNTIFS('دفتر الحركات المالية'!G:G,A${row},'دفتر الحركات المالية'!C:C,"💰 استحقاق مصروف"))`
    );
    // عدد الدفعات
    sheet.getRange(row, 7).setFormula(
      `=IF(A${row}="","",COUNTIFS('دفتر الحركات المالية'!G:G,A${row},'دفتر الحركات المالية'!C:C,"💸 دفعة مصروف"))`
    );
  ...
  sheet.getRange(2, 3, 99, 3).setNumberFormat('$#,##0.00');
  sheet.getRange(2, 8, 99, 1).setNumberFormat('0.0%');
}
...
function createCashFlowSheet(ss) {
  ...
  // 🔹 قائمة الشهور من عمود "الشهر" الجديد = T
  sheet.getRange('A2').setFormula(
    `=SORT(UNIQUE(FILTER('دفتر الحركات المالية'!T2:T,'دفتر الحركات المالية'!T2:T<>"")))`
  );
  
  for (let row = 2; row <= 100; row++) {
    // 🔹 إجمالي الاستحقاقات (مصروفات) في الشهر
    sheet.getRange(row, 2).setFormula(
      `=IF(A${row}="","",SUMIFS('دفتر الحركات المالية'!J:J,'دفتر الحركات المالية'!T:T,A${row},'دفتر الحركات المالية'!C:C,"💰 استحقاق مصروف"))`
    );
    
    // 🔹 إجمالي الدفعات (مصروفات)
    sheet.getRange(row, 3).setFormula(
      `=IF(A${row}="","",SUMIFS('دفتر الحركات المالية'!K:K,'دفتر الحركات المالية'!T:T,A${row},'دفتر الحركات المالية'!C:C,"💸 دفعة مصروف"))`
    );
    
    // 🔹 إجمالي الإيرادات المحصلة
    sheet.getRange(row, 4).setFormula(
      `=IF(A${row}="","",SUMIFS('دفتر الحركات المالية'!K:K,'دفتر الحركات المالية'!T:T,A${row},'دفتر الحركات المالية'!C:C,"✅ تحصيل إيراد"))`
    );
    
    // 🔹 صافي التدفق = إيرادات - دفعات
    sheet.getRange(row, 5).setFormula(
      `=IF(A${row}="","",D${row}-C${row})`
    );
    
    // 🔹 التدفق التراكمي
    sheet.getRange(row, 6).setFormula(
      `=IF(A${row}="","",SUM($E$2:E${row}))`
    );
  }
  
  sheet.getRange(2, 2, 99, 5).setNumberFormat('$#,##0.00');
}
...
function createDashboardSheet(ss) {
  ...
  const metrics = [
    ['', '', ''],                                  // 3
    ['💰 المؤشرات المالية', '', ''],              // 4
    // إجمالي الاستحقاقات (مصروفات) من J:J
    ['إجمالي الاستحقاقات (مصروفات)', 
      '=SUMIFS(\'دفتر الحركات المالية\'!J:J,\'دفتر الحركات المالية\'!C:C,"💰 استحقاق مصروف")',
      'USD'
    ],                                            // 5
    // إجمالي المدفوع (مصروفات) من K:K
    ['إجمالي المدفوع (مصروفات)',    
      '=SUMIFS(\'دفتر الحركات المالية\'!K:K,\'دفتر الحركات المالية\'!C:C,"💸 دفعة مصروف")',
      'USD'
    ],                                            // 6
    ['🆕 المصروفات المباشرة',       
      '=SUMIFS(\'دفتر الحركات المالية\'!J:J,\'دفتر الحركات المالية\'!D:D,"مصروفات مباشرة")',
      'USD'
    ],                                            // 7
    ['🆕 المصروفات العمومية',       
      '=SUMIFS(\'دفتر الحركات المالية\'!J:J,\'دفتر الحركات المالية\'!D:D,"مصروفات عمومية")',
      'USD'
    ],                                            // 8
    ...
    ['إجمالي الإيرادات المحصلة',  
      '=SUMIFS(\'دفتر الحركات المالية\'!K:K,\'دفتر الحركات المالية\'!C:C,"✅ تحصيل إيراد")',
      'USD'
    ],                                            // 10
    ['', '', ''],                                 // 11
    ['📈 إحصائيات', '', ''],                     // 12
    ['عدد المشاريع النشطة', 
      '=COUNTIF(\'قاعدة بيانات المشاريع\'!O:O,"جاري التنفيذ")',
      'مشروع'
    ],                                            // 13
    ['عدد الموردين',        
      '=COUNTA(\'قاعدة بيانات الموردين\'!A:A)-1',
      'مورد'
    ],                                            // 14
    ['عدد الاستحقاقات المعلقة', 
      '=COUNTIF(\'دفتر الحركات المالية\'!R:R,"معلق")',
      'استحقاق'
    ],                                            // 15
    ['عدد الدفعات هذا الشهر', 
      '=COUNTIFS(\'دفتر الحركات المالية\'!C:C,"💸 دفعة مصروف",\'دفتر الحركات المالية\'!S:S,TEXT(TODAY(),"YYYY-MM"))',
      'دفعة'
    ],                                            // 16
    ['', '', ''],                                 // 17
    ['💵 السيولة المتاحة (بنك + خزنة)', '', ''], // 18
    // أرصدة البنك والخزنة (نفترض أن الرصيد في العمود F في كل شيت)
    ['رصيد حساب البنك - دولار', 
      '=IFERROR(LOOKUP(2,1/(\'حساب البنك - دولار\'!A:A<>""),\'حساب البنك - دولار\'!F:F),0)',
      'USD'
    ],                                            // 19
    ['رصيد حساب البنك - ليرة', 
      '=IFERROR(LOOKUP(2,1/(\'حساب البنك - ليرة\'!A:A<>""),\'حساب البنك - ليرة\'!F:F),0)',
      'TRY'
    ],                                            // 20
    ['رصيد خزنة العهدة - دولار', 
      '=IFERROR(LOOKUP(2,1/(\'خزنة العهدة - دولار\'!A:A<>""),\'خزنة العهدة - دولار\'!F:F),0)',
      'USD'
    ],                                            // 21
    ['رصيد خزنة العهدة - ليرة', 
      '=IFERROR(LOOKUP(2,1/(\'خزنة العهدة - ليرة\'!A:A<>""),\'خزنة العهدة - ليرة\'!F:F),0)',
      'TRY'
    ],                                            // 22
    ['', '', ''],                                 // 23
    ...
    ['إجمالي القروض المستلمة من الممولين', 
      '=IFERROR(SUMIFS(\'دفتر الحركات المالية\'!K:K,\'دفتر الحركات المالية\'!C:C,"🏦 تمويل"),0)',
      'USD'
    ],                                            // 29
    // إجمالي سداد القروض من نوع الحركة 💳 سداد تمويل
    ['إجمالي سداد القروض', 
      '=IFERROR(SUMIFS(\'دفتر الحركات المالية\'!J:J,\'دفتر الحركات المالية\'!C:C,"💳 سداد تمويل"),0)',
      'USD'
    ],                                            // 30
    ...
    ['إجمالي الذمم المدينة على العملاء (إيرادات لم تُحصَّل)', 
      '=IFERROR(SUM(FILTER(\'تقرير الإيرادات\'!F:F,\'تقرير الإيرادات\'!F:F>0)),0)',
      'USD'
    ],                                            // 32
    // ذمم دائنة لصالح الموردين = مجموع "الرصيد الحالي" الموجب في تقرير الموردين (عمود F)
    ['إجمالي الذمم الدائنة لصالح الموردين', 
      '=IFERROR(SUM(FILTER(\'تقرير الموردين\'!F:F,\'تقرير الموردين\'!F:F>0)),0)',
      'USD'
    ]                                             // 33
  ];
...
function createItemsSheet(ss) {
  let sheet = ss.getSheetByName('قاعدة بيانات البنود');
  if (!sheet) {
    sheet = ss.insertSheet('قاعدة بيانات البنود');
  }

  const headers = ['اسم البند', 'قائمة طبيعة الحركة', 'قائمة تصنيف الحركة'];
  sheet.getRange(1, 1, 1, headers.length)
    .setValues([headers])
    .setBackground('#33691e')
    .setFontColor('#ffffff')
...
// ==================== شيتات البنك وخزنة العهدة (دولار / ليرة / بطاقة) ====================
